...
My game is a Super Mario Bros. themed Pac-man game.
Move around the map using the arrow keys to collect all of the coins
...
they take damage and decrease their speed. You can level up infinitely, but if you die once,
you go back to level one. There is an option to replay the game. The game resets each time you level up.
...
wall = Wall(135, 125, 15, 145, GREEN)
wall_list.add(wall)
all_sprites_list.add(wall)
...
def intro_screen():
    ...
    while not done:
        ...
        screen.blit(text_1, [200, 245])
        screen.blit(text_2, [250, 290])
        screen.blit(text_3, [260, 320])
...
def clear():
    # speed up goomba and reset position
    redgoomba.rect.center = (650, 650)
    redgoomba.speed_up()
    goomba_list.add(redgoomba)

    orangegoomba.rect.center = (100, 100)
    orangegoomba.speed_up()
    goomba_list.add(orangegoomba)

    browngoomba.rect.center = (650, 100)
    browngoomba.speed_up()
    goomba_list.add(browngoomba)
    ...
    for i in range(300):
        ...
        my_font = pygame.font.Font("game.ttf", 20)
        text = my_font.render("LEVEL CLEARED!", True, GREEN)

        screen.blit(text, [265, 345])
...
def instructions_screen():
    ...
    while not done:
        ...
        text = special_font.render("INSTRUCTIONS:", True, WHITE)
        text_1 = my_font.render("Travel around the map using the arrow keys to collect coins.", True, WHITE)
        ...
        screen.blit(text_2, [210, 370])
        screen.blit(text_8, [180, 400])
        screen.blit(text_3, [185, 430])
        screen.blit(text_4, [80, 460])
        screen.blit(text_5, [255, 560])
        screen.blit(text_6, [215, 600])
...
def game_over():
    ...
    background_music.stop()
    my_font = pygame.font.Font("game.ttf", 30)
    game_over_sound.play(0)
    end_game = False

    # reset goomba speed and position
    redgoomba.rect.center = (650, 650)
    redgoomba.reset()
    goomba_list.add(redgoomba)
...
while not done:
    ...
    if pygame.time.get_ticks() % 120 < 100:
        screen.blit(coin_image, [100, 340])
